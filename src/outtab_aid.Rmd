...
survfunc <- function(time, event, eventcr = NULL, eventname, rep = FALSE) {
  ...
  out[1, 1] <- eventname
  colnames(out) <- c("Outcome", "Model", levels(rsdata$scream_aid))
  ...
  if (!rep) {
    ...
    if (!is.null(eventcr)) {
      ...
      modvarscr <- modvarscr[!modvarscr %in% c("scream_anemia_cr_Yes", "scream_id_cr_Yes")]
...
default_kable(outall,
  font_size = 6,
  caption = "Association between A/ID and outcomes"
)
```
