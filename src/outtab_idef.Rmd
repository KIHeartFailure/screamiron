...
survfunc <- function(time, event, eventname, rep = FALSE) {
  ...
  modvarstmp <- modvars[!modvars %in% c("scream_id", "shf_ef_cat")]
  if (!rep) {
    # cox regressions
    ## crude
    ...
    for (i in seq_along(levsef)) {
      ...
      if (i == 1) {
        amodnoint <- with(imprsdatalab, coxph(formula(paste0(
          "Surv(", time, ",", event, " == 'Yes') ~  scream_id + relevel(shf_ef_cat, ref = '", levsef[i], "') +",
          paste(modvars, collapse = " + ")
        ))))
        ...
        out[1, 9] <- fn(int$result[4], dig = 3, p = TRUE)
      }
    }
  }
  if (rep) {
    ...
    for (i in seq_along(levsef)) {
      ...
      if (i == 1) {
        amodnoint <- with(imprsdatalab, glm.nb(formula(paste0(
          event, " ~ scream_id + relevel(shf_ef_cat, ref = '", levsef[i], "') + offset(log(", time, ")) +",
          paste(modvarstmp, collapse = " + ")
        ))))
...
write.xlsx(outall, paste0("./output/tabs/out_idef_", Sys.Date(), ".xlsx"), rowNames = FALSE, overwrite = TRUE)

colnames(outall) <- c("Outcome", "Model", rep(c("No ID", "ID"), 3), "p interaction")

default_kable(outall,
  font_size = 6,
  caption = "Association between ID and outcomes by EF"
