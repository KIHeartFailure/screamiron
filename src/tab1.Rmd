...
write.xlsx(tab1 %>%
  select(Variable, Missing:p), paste0("./output/tabs/tab1_", Sys.Date(), ".xlsx"), rowNames = FALSE, overwrite = TRUE)
...
colnames(tab1) <- sanitize_text(c(
  "Variables", "Missing (%)", levels(rsdata$scream_aid), "p-value"
))

footnote(
  default_kable(tab1,
    font_size = 6,
    caption = "Baseline characteristics by A/ID",
    longtable = TRUE,
    escape = FALSE
  ) %>%
    landscape(),
