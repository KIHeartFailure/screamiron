...
survfunc <- function(time, event, eventcr = NULL, eventname, rep = FALSE) {
  ...
  modvarstmp <- modvarsfcm[!modvarsfcm %in% c("scream_id")]
  if (!rep) {
    # cox regressions
    ## crude
...
write.xlsx(outall, paste0("./output/tabs/out_idfcm_", Sys.Date(), ".xlsx"), rowNames = FALSE, overwrite = TRUE)

default_kable(outall,
  font_size = 6,
  caption = "Association between ID and outcomes - Consistency analysis with FCM"
)
```
