```{r outtabid, cache=cacheon}
survfunc <- function(time, event, eventcr = NULL, eventname, rep = FALSE) {
  out <- data.frame(matrix(NA, ncol = 4, nrow = ifelse(is.null(eventcr), 3, 4)))

  out[1, 1] <- eventname
  colnames(out) <- c("Outcome", "Model", levels(rsdatalab$scream_id))
  ...
  modvarstmp <- modvars[!modvars %in% c("scream_id")]
  if (!rep) {
    # cox regressions
    ## crude
    ...
    ## crude
    mod <- summary(coxph(formula(paste0("Surv(", time, ",", event, "=='Yes') ~ scream_id")),
      data = rsdatalab
    ))
    ...
    if (!is.null(eventcr)) {
      ...
      modvarscr <- modvarscr[!modvarscr %in% c("scream_id_cr_Yes", "scream_aid_cr_ApIDm", "scream_aid_cr_AmIDp", "scream_aid_cr_ApIDp")]

      mod <- crr_mids_hf(
        event = eventcr,
        time = time,
        xvars = c("scream_id_cr_Yes", modvarscr),
        data = imprsdatalab,
      )
...
write.xlsx(outall, paste0("./output/tabs/out_id_", Sys.Date(), ".xlsx"), rowNames = FALSE, overwrite = TRUE)

default_kable(outall,
  font_size = 6,
  caption = "Association between ID and outcomes"
)
```
